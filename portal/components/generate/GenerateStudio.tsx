...
export default function GenerateStudio() {
  ...
  const GOLDEN_CAT: Record<GenKind, string> = { sticker: "stickers", meme: "memes", product: "merch", social: "social" };

  async function makeGolden() {
    if (blessing || !img) return;
    setErr(null); setBlessing(true);
    try {
      const res = await fetch("/api/golden-pr", {
        method: "POST",
        headers: { "content-type": "application/json", "x-aitx-code": code },
        body: JSON.stringify({ imageBase64: img, recipe, category: GOLDEN_CAT[kind], name: `${kind}-${brief}` }),
      });
      const data = await res.json();
      if (!res.ok) { setErr(data.error ?? "Could not open the PR."); return; }
  ...
  async function generate() {
    setBusy(true); setErr(null); setImg(null); setRecipe(null); setPr(null);
    try {
        ...
        {img ? (
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap gap-2">
              ...
              {pr ? (
                <a href={pr.url} target="_blank" rel="noreferrer" className="inline-block rounded-md px-4 py-2 text-center text-sm font-semibold text-white" style={{ background: "#4a9d3a" }}>✓ PR #{pr.number} opened →</a>
              ) : (
                <button type="button" onClick={makeGolden} disabled={blessing}
                  className="inline-block rounded-md border border-[color:var(--orange)] px-4 py-2 text-center text-sm font-semibold text-[color:var(--orange-deep)] hover:bg-[color:var(--orange)]/10 disabled:opacity-60">
                  {blessing ? "Opening a PR…" : "★ Love it — make it a golden"}
                </button>
              )}
            </div>
            {pr ? <p className="text-xs text-[color:var(--muted)]">A human merges the PR to bless it into the brand universe.</p> : null}
